...
class InstagramOSINT:
    ...
    def _get_random_proxy(self) -> Optional[Dict[str, str]]:
        ...
        Returns: Optional proxy configuration
        """
        return None
    ...
    def check_phone(self, phone: str) -> Dict[str, any]:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if not phone.startswith(('1', '91')):
                    formatted_phone = f"91{phone}" if len(phone) == 10 else phone

                # First try the password reset endpoint
                ...
                if reset_response.status_code == 200:
                    try:
                        json_response = reset_response.json()
                        if 'message' in json_response:
                            if 'sent' in json_response['message'].lower() or 'found' in json_response['message'].lower():
                                result['exists'] = True
                                return result
                    except json.JSONDecodeError:
                        pass
                ...
                lookup_url = f"{self.ajax_url}/users/lookup/"
                lookup_data = {
                ...
                if lookup_response.status_code == 200:
                    try:
                        ...
                        if json_response.get('status') == 'ok' or json_response.get('user'):
                            result['exists'] = True
                            if json_response.get('user'):
                                result['username'] = json_response['user'].get('username')
                            return result
                ...
                # Try the account recovery endpoint as last resort
                ...
                if recovery_response.status_code == 200:
                    try:
                        ...
                        if json_response.get('status') == 'ok' or 'sent' in str(json_response.get('message', '')).lower():
                            result['exists'] = True
                            return result
                    except json.JSONDecodeError:
                        if attempt == self.max_retries - 1:
                            result['error'] = "Could not parse Instagram response"
                        continue

            except requests.RequestException as e:
                if attempt == self.max_retries - 1:
                    result['error'] = f"Request failed after {self.max_retries} attempts: {str(e)}"
                continue
            except Exception as e:
                if attempt == self.max_retries - 1:
