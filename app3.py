    ...
    def check_phone(self, phone: str) -> Dict[str, any]:
        ...
        try:
            ...
            if not phone.startswith('1') and len(phone) > 10:
                ...
                formatted_phone = f"1{phone}" if len(phone) == 10 else phone
            ...
            if response.status_code == 200:
                try:
                    json_response = response.json()
                    if json_response.get('status') == 'ok':
                        if json_response.get('obfuscated_phone'):
                            result['exists'] = True
                            result['obfuscated_phone'] = json_response.get('obfuscated_phone')
                        if json_response.get('user'):
                            result['username'] = json_response['user'].get('username')
                    elif json_response.get('message'):
                ...
                try:
                    alt_url = f"{self.base_url}/api/v1/accounts/send_recovery_flow_options/"
                    data.update({
                        'phone_number': formatted_phone,
                        'recovery_flow': 'user_lookup'
                    })
                    ...
                    if alt_response.status_code == 200:
                        ...
                        result['error'] = f"All attempts failed. Status codes: {response.status_code}, {alt_response.status_code}"
                except Exception as e:
