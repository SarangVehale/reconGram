...
class InstagramOSINT:
    ...
    def _extract_username_from_response(self, response_data: Dict) -> Optional[str]:
        """Extract username from various response formats"""
        if isinstance(response_data, dict):
            ...
            if 'user' in response_data and isinstance(response_data['user'], dict):
                return response_data['user'].get('username')
            
            # Nested in data object
            if 'data' in response_data and isinstance(response_data['data'], dict):
                return response_data['data'].get('username')
            
            # Check message field for username
            if 'message' in response_data:
                username_match = re.search(r'@([\w._]+)', response_data['message'])
                if username_match:
...
def print_banner():
    ...
    banner = f"""
    {Fore.CYAN}╔══════════════════════════════════════════╗
    ║       {Fore.YELLOW}Instagram Advanced OSINT Tool      {Fore.CYAN}║
    ║    {Fore.YELLOW}Username/Email/Phone Number Checker   {Fore.CYAN}║
    ╚══════════════════════════════════════════╝
    {Fore.RED}
